...
export default function TestimonialCard({ name, role, review, rating, image }) {
  return (
    <div className="relative bg-white/5 backdrop-blur-sm rounded-2xl p-8 border border-white/10 hover:border-teal-400/30 transition-all duration-300 hover:transform hover:-translate-y-1">
      ...
      <div className="relative z-10">
        ...
        <div className="flex gap-1 mb-4 justify-center">
          {[...Array(5)].map((_, i) => (
            <svg 
              key={i}
              className={`w-5 h-5 ${i < rating ? 'text-yellow-400 fill-current' : 'text-white/20 fill-current'}`}
              viewBox="0 0 20 20"
            >
